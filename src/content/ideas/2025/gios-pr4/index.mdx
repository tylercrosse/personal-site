...
import protobufExample from "./protobuf-placeholder.png";
...
A DFS is the backbone of modern cloud storage and collaboration. It allows multiple, geographically dispersed clients to access and modify a shared set of files as if they were stored on a single, local machine. For my final project in Georgia Tech's [CS 6200: Graduate Introduction to Operating Systems (GIOS)](https://omscs.gatech.edu/cs-6200-introduction-operating-systems), I was tasked with building one from the ground up in C++.

In the [first project](/ideas/2025/gios-pr1/), I built a simple file system with a single client and server. In the [second project](/ideas/2025/gios-pr3/), I extended it to add a proxy and cache server. This project is the final step in the journey, building a distributed file system with multiple clients and servers. This project was a fantastic 
...
<Figure
  caption="A simple example of a Protocol Buffers message. The `name` field is encoded as a string with tag number 1, and the `age` field is encoded as an integer with tag number 2."
  src={protobufExample}
  alt="A simple example of a Protocol Buffers message. The `name` field is encoded as a string with tag number 1, and the `age` field is encoded as an integer with tag number 2."
/>
...
| Consistency Model   | Guarantee                                                | Pro (Performance / Availability) | Con (Simplicity / Correctness) |
| ------------------- | -------------------------------------------------------- | -------------------------------- | ------------------------------ |
| **Strong**          | Reads always see the most recently completed write.      | Slowest / Lowest                 | Easiest to reason about        |
| **Sequential**      | Operations appear in some global order.                  | Medium                           | Complex to implement           |
| **Eventual**        | Replicas will eventually converge if writes stop.        | Fastest / Highest                | Hardest to reason about        |
| **This Project**    | Writes are serialized; reads may be temporarily stale.   | High (for reads)                 | Balanced trade-off             |

*   **Strong Consistency:** This is the most intuitive model, behaving like a single machine. Any read will always return the result of the most recently completed write. Think of a bank transaction: you expect your balance to be updated everywhere, immediately. This is the easiest to reason about, but it's often the slowest, as it may require complex locking or coordination across the network for every single operation, sacrificing availability if the network has problems.

*   **Eventual Consistency:** This is the weakest model, prioritizing availability and speed. It guarantees that if no new updates are made, all replicas will *eventually* converge to the same value, but it doesn't say *when*. Think of DNS or updating a social media profile; your new profile picture will show up for everyone, but it might take a few seconds or minutes to propagate. This model offers the best performance, as clients can continue to read and write from local replicas without waiting, but it means developers have to handle the possibility of working with stale data.

This project implemented a **weak consistency model** inspired by the **Andrew File System (AFS)**, which strikes a pragmatic balance between the two extremes. In this model:
- Clients aggressively **cache** copies of files locally for fast access.
- When a client wants to *modify* a file, it must acquire a **write lock** from the server. This ensures only one client can write to a file at any given time, preventing the "lost update" anomaly where concurrent edits could overwrite each other.
- The client determines if its cached file is stale by comparing the file's metadata (like a timestamp or checksum) against the server's version before using it.
- When a client closes a modified file, it uploads the new version to the server. The server then sends out **cache invalidation** callbacks to all other clients that have a copy of that file, telling them their version is now stale and must be re-fetched on the next access.

This "write-on-close" strategy provides the ideal trade-off for this project: it makes reads snappy by serving them from the local cache, while still providing strong enough guarantees to prevent data corruption and keep data reasonably fresh across all clients. It accepts that a client might read slightly stale data in the short window between another client's write and the invalidation message, a perfectly acceptable compromise for this type of application.
