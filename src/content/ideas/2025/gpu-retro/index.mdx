...
status: 'complete'
type: 'retro'
tags: ['GPU', 'CUDA', 'computer-architecture', 'distributed-systems', 'parallel-algorithms', 'compilers', 'machine-learning', 'simulation', 'computer-science', 'learning-in-public', 'MSCS']
category: ['mscs']
draft: false
audience: 'All'
...
import compArchQuantImg from "../../../books/comp-arch-quant.jpg";
...
- **Projects 1 & 2:** [CUDA Fundamentals: Tiled Matrix Multiply & Bitonic Sort](/ideas/2025/gpu-p1p2/)
- **Projects 3 & 4:** [GPU Simulation: Warp Scheduling & Compute/Tensor Cores](/ideas/2025/gpu-p3p4/)
- **Project 5:** [Static Analysis: Detecting Branch Divergence](/ideas/2025/gpu-p5/)
- **Extra Project:** [FlashAttention & LLM Inference on GPUs](/ideas/2025/gpu-p6/)
...
<Figure
  ...
  caption="Each CUDA thread has a unique position in a grid of blocks. threadIdx gives its position within its block, and blockIdx gives the block's position in the grid. Together they identify a unique element to work on."
  width={640}
/>
...
- **Shared memory** (`__shared__`): A fast, explicitly managed scratchpad memory shared by all threads in a block. This is the GPU programmer's main tool for data reuse. It usually has much lower latency and higher bandwidth than global memory, but exact gains depend on architecture and access pattern.
...
A recurring pattern in high-performance CUDA kernels is: load from global into shared, compute, then write back to global. The tiled matrix multiplication in [Project 1](/ideas/2025/gpu-p1p2/) is the canonical example.
...
The CCWS insight is that latency hiding (which benefits from more concurrent warps) and cache locality (which benefits from fewer) are in fundamental tension. The right balance is workload-dependent. [Project 3](/ideas/2025/gpu-p3p4/) involved implementing all three of these policies in a cycle-level GPU simulator.
...
This kind of analysis is how GPU architects design and validate new hardware before it's built. Academic researchers use simulators to prototype and evaluate new architectural ideas. [Projects 3 and 4](/ideas/2025/gpu-p3p4/) used **Macsim**, a trace-driven cycle-level simulator developed at Georgia Tech's HPArch lab.
...
<Figure
  src={rooflineImg}
  alt="Roofline model"
  caption="The Roofline model shows where a workload sits relative to the compute and memory bandwidth ceilings. Workloads to the left of the 'ridge point' are memory bound, while those to the right are compute bound."
  width={640}
/>

Tiled matrix multiplication ([Project 1](/ideas/2025/gpu-p1p2/)) is a classic example of using shared memory to increase arithmetic intensity and push a workload from memory-bound to compute-bound. The FlashAttention algorithm ([Extra Project](/ideas/2025/gpu-p6/)) uses the same principle at much larger scale.
...
<Figure
  src={gemmImg}
  alt="GEMM operation"
  caption="GEMM (General Matrix Multiply) is the workhorse of deep learning. Every linear layer, every attention computation, every convolution reduces to one or more GEMM operations. A central figure of merit is arithmetic intensity: FLOPs per byte."
  width={640}
/>
...
<Figure
  ...
  caption="A tensor core computes D = A*B + C as a single instruction, where A, B, C, D are small matrices (e.g., 4×8 times 8×4). Throughput gains over regular CUDA cores can be large for compatible GEMM workloads, but the exact factor depends on architecture, precision mode, and kernel shape."
  width={640}
/>
...
- **P1 (Tiled Matrix Multiply)** and **P2 (Bitonic Sort)**: Writing real CUDA kernels, managing shared memory, optimizing for memory coalescing and occupancy. A hands-on introduction to the hardware concepts from the first half of the course. [Full write-up.](/ideas/2025/gpu-p1p2/)
- **P3 (Warp Scheduling Simulator)** and **P4 (Compute/Tensor Core Simulation)**: Implementing warp scheduling policies and execution modeling inside a cycle-level simulator. Made the microarchitectural concepts concrete in a way that reading never fully does. [Full write-up.](/ideas/2025/gpu-p3p4/)
- **P5 (Branch Divergence Detection)**: A static analysis pass that detects divergent branches by propagating thread-ID taint through a SASS control flow graph. Tied together the compiler and architecture topics from the second half of the course. [Full write-up.](/ideas/2025/gpu-p5/)
- **Extra Project (FlashAttention)**: A progressive implementation of self-attention, FlashAttention-2, and a KV-cached decode kernel inside a real GPT-2 inference pipeline. The capstone. [Full write-up.](/ideas/2025/gpu-p6/)
