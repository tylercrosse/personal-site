...
status: 'complete'
type: 'project'
tags: ['GPU', 'branch-divergence', 'static-analysis', 'dataflow-analysis', 'SASS', 'compilers', 'control-flow-graph', 'MSCS']
category: ['projects']
draft: false
audience: 'Technical'
...
On a CPU with branch prediction and out-of-order execution, this is practically free. On a GPU, it can cut your throughput roughly in half.
...
When all threads in a warp follow the same control flow path, this works beautifully — perfect utilization. The problem arises at branches.

<Figure
  src={divbImg}
  alt="Branch divergence in a warp"
  caption="An if-else statement where the condition depends on threadIdx. Threads 0 and 1 take the 'then' path (BB2); threads 2 and 3 take the 'else' path (BB3). The warp must serialize these two paths — all threads wait while each sub-group runs its instructions behind a mask."
  width={640}
/>

When threads in a warp take different branches, the hardware uses an **active mask** to track which threads should be executing at each instruction. The warp first executes one branch path with some threads masked off (inactive), then executes the other branch path with the opposite mask. The total work is the sum of both paths — any throughput benefit of parallelism within a divergent branch is lost.
...
For branches with very small bodies, compilers often eliminate the branch entirely using **predication**. Instead of branching, both sides of the if-else are converted to predicated instructions — instructions that execute conditionally based on a predicate register, but are *always fetched*.
...
The reconvergence point — called the **immediate post-dominator** in compiler terms — is the first basic block that all paths from a branch must eventually pass through. Identifying it is a compiler job. Before the GPU even loads the program, the compiler needs to mark which branches are divergent and annotate the binary with reconvergence information. This is why detecting divergent branches at compile time matters: it's not just a performance concern, it's a correctness requirement for the hardware support mechanism.
...
<Figure
  ...
  caption="CUDA compilation splits source code into host and device components. The device code path goes through Clang (frontend), LLVM IR optimizations, PTX code generation (virtual ISA, architecture-independent), and finally SASS generation (real machine assembly, architecture-specific). Branch divergence analysis happens at the IR or PTX level."
  width={640}
/>

Understanding what Project 5 analyzes requires understanding where SASS lives in this stack.
...
1. Identify **leaders**: the first instruction; any instruction that is the target of a branch; any instruction immediately following a branch
...
<Figure
  ...
  caption="Reaching definitions analysis. A definition 'd' of variable v 'reaches' a program point p if there exists a path from d to p that does not pass through another definition of v. The GEN set contains definitions produced in a block; the KILL set contains definitions that are overwritten."
  width={640}
/>
...
The key subtlety: the same register name can have multiple definitions at different points in the CFG (not SSA form — register names are reused). You need to track which definition is "live" at each point, which is exactly what reaching definitions computes.

### Taint Propagation from Thread IDs

With the def-use table in hand, task 2 is identifying which instructions are "tainted" — thread-ID-dependent.
...
Task 3 is the payoff: from the tainted instruction set, find all **branch instructions**. A branch instruction that is tainted is a potential divergence point — its condition depends on thread ID, so different threads in the same warp may evaluate it differently.
...
In SASS, branches are instructions like `BRA` or `BRX` (indirect branch). The opcode check is simple; the hard work was done in the def-use construction and taint propagation steps.
...
- **Warp specialization**: An advanced optimization where the compiler generates specialized code for "all threads take this branch" vs. "threads are split" — avoiding the masking overhead in the common case
- **Developer feedback**: Modern profilers can report detected divergent branches; the detection logic is dataflow analysis

The deeper lesson: the same classical compiler techniques that were developed for vectorizing sequential loops in the 1980s — reaching definitions, def-use chains, iterative fixed-point analysis — are directly applicable to GPU branch divergence detection. The hardware model changed dramatically; the analysis framework did not.
