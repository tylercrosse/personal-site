...
Imagine a wildly popular news website. On a big story day, millions of users are all trying to load the same front-page article and its images. The main web server, buried deep in a datacenter, gets hammered with identical requests, spending its cycles reading the same files from disk over and over. For users on the other side of the world, the experience is slow as data travels thousands of miles. This is a classic scalability bottleneck. How do you serve popular content to a global audience, quickly and efficiently, without overwhelming your origin server?
...
For my third project in Georgia Tech's OS course, the challenge was to build exactly this system: a cooperating proxy and cache. This wasn't about threads within a single program anymore; it was about two distinct processes that had to communicate and coordinate. This forced a deep dive into one of the most fundamental topics in operating systems: **Inter-Process Communication (IPC)**. How do you pass massive amounts of file data from a cache process to a proxy process with minimal overhead? This post explores that journey, and the powerful, tricky, and essential world of IPC that makes systems like this possible.
...
In a production system like Nginx or a Content Delivery Network (CDN), the proxy and cache logic are typically part of the same highly-optimized application. For this project, however, the two were explicitly separated into distinct processes to create a focused challenge in systems programming.
...
This artificial separation is the heart of the project. By splitting these tightly-coupled roles into two programs, the primary engineering problem becomes: **how do you build a high-bandwidth, low-latency communication channel between them?** This forces a deep dive into Inter-Process Communication, making it the central lesson.
...
A key design decision mandated by the project was that **the proxy is responsible for creating and destroying the IPC resources**. From the cache's perspective, this follows the "don't mess with my memory" adage. It makes the cache a more robust service, capable of handling multiple proxy clients without being brought down by a single misbehaving one. If a proxy crashes, it's responsible for its own mess, and the cache can continue serving other clients.
