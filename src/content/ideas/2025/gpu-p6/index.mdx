...
status: 'complete'
type: 'project'
tags: ['GPU', 'FlashAttention', 'LLM', 'CUDA', 'transformers', 'attention', 'GPT-2', 'tensor-cores', 'KV-cache', 'inference', 'MSCS']
category: ['projects']
draft: false
audience: 'Technical'
...
It ties together everything from the course: CUDA kernel writing and profiling (Projects 1–2), understanding of latency and throughput tradeoffs (Projects 3–4), and the GPU's relationship to the ML software stack (Module 12). It's the most ambitious project in the course, and it's the most satisfying one to complete.

> [!note]
> This post focuses on the concepts behind the project. For a broader course overview, see: [GPU Hardware and Software: A Retrospective](/ideas/2026/gpu-retro/).
...
In a language model, Q, K, and V are all derived from the same input sequence by multiplying with learned weight matrices ($W_Q$, $W_K$, $W_V$). This is **self-attention** — the sequence attending to itself, each position weighting all other positions.

<Figure
  src={gemmImg}
  alt="GEMM operation behind attention"
  caption="The QK^T computation is a GEMM: A=[batch, seq, d_head] times B^T=[batch, d_head, seq] produces S=[batch, seq, seq]. For seq=4096, this is a 4096×4096 matrix per head per batch element — the memory bottleneck."
  width={640}
/>
...
The key point about MHA on a GPU: all heads can be computed as a single batched matrix multiplication. The head dimension just becomes an extra batch dimension. No sequential loop over heads required.

### Causal Masking

For autoregressive text generation, a token at position *i* should only be able to attend to tokens at positions ≤ *i* — it can't look at future tokens that haven't been generated yet. This is enforced by a **causal mask**: before the softmax, set the upper-triangular portion of the score matrix to $-\infty$. After softmax, those positions become 0 (zero weight). Tokens at future positions are effectively ignored.
...
The problem: for large input values, $e^{x_i}$ overflows float32 (max representable ~3.4×10³⁸). Attention scores can easily reach 100+ for long sequences with large embeddings. A score of 90 gives $e^{90} \approx 10^{39}$ — overflow.
...
This is mathematically identical (the $e^{-x_{\max}}$ terms cancel) but numerically stable — the largest exponentiated value is $e^0 = 1$.

In CUDA, computing the row maximum requires a **parallel reduction** within a thread block. The pattern is log₂(N) passes: in each pass, half the threads compare pairs of values and write the winner into shared memory, then sync. After log₂(N) passes, thread 0 holds the block maximum.
...
Both use the tiled GEMM pattern from Project 1: divide the output matrix into tiles, cooperatively load tiles of the input matrices into shared memory, compute partial dot products, accumulate, advance to the next tile.
...
1. Compute $S = QK^T/\sqrt{d}$ — writes a [batch, heads, N, N] tensor to global memory
2. Apply causal mask — reads and writes the full [batch, heads, N, N] tensor
3. Compute softmax — reads and writes the full [batch, heads, N, N] tensor
4. Compute $O = \text{softmax}(S) \cdot V$ — reads the full [batch, heads, N, N] tensor

Steps 1-4 each touch O(N²) data in global memory. For N=4096, that's four passes over 32 MB (per head, per batch element). The arithmetic intensity is terrible — you're doing minimal computation per byte loaded.
...
The key mathematical insight: softmax can be computed **incrementally** as you see new values, without knowing the final maximum or sum in advance.

If you've already computed a partial softmax for the first *j* elements, and you see a new element $x_{j+1}$, you can update the running maximum and running sum to incorporate the new value — and *correct* the partial output accordingly. This is the "online softmax" algorithm.

FlashAttention applies this idea at tile granularity:

1. Divide Q into blocks of $B_r$ rows; divide K and V into blocks of $B_c$ rows
...
The critical result: at no point is the full N×N score matrix ever written to global memory. The only data that touches global memory is Q, K, V (read once each) and O (written once). Memory complexity drops from O(N²) to O(N).
...
The speedup grows dramatically with sequence length — exactly as expected from the O(N²) → O(N) memory reduction. At seq=4096, the naive implementation is thrashing the global memory bus; FlashAttention avoids it.
...
**Prefill**: Process the entire input prompt at once to produce the first output token. The full N-token sequence runs through all layers. This is a GEMM-heavy operation — all of FlashAttention applies here.

**Decode**: Generate subsequent tokens one at a time. Each new token attends to all previous tokens, but *it's just one query* attending to N keys and values. The attention score computation degenerates from a matrix-matrix multiply to a **matrix-vector multiply** (GEMV). The character of the computation changes completely.
...
Without the KV cache, each decode step would need to re-process the entire sequence from scratch — O(N) work per token, O(N²) total for generating a full response. With the KV cache, each step is O(N) work but you avoid recomputing K and V for all previous tokens.

The KV cache has a memory cost: for GPT-2 (12 layers, 12 heads, head_dim=64), generating 100 tokens on top of a 1024-token prompt requires storing 12 × 2 × 1124 × 64 = ~1.7 million fp16 values per batch element. For large models at scale (GPT-4 class, long contexts, large batches), KV cache memory often becomes the binding constraint on batch size.

> The state-of-the-art solution to KV cache memory fragmentation is [PagedAttention](https://arxiv.org/abs/2309.06180) — treating the KV cache like virtual memory with pages, rather than pre-allocating contiguous blocks. But that's beyond the scope of this project.

### The Decode Kernel

The FlashAttention CUDA kernel needs modification for the decode phase. When Q has sequence length 1 (one query), the outer loop over Q-blocks collapses to a single iteration — you process the one query against all K/V blocks.
...
If I had to pick one project that ties together everything from the course, it's this one.
...
It also demonstrates something broader about GPU programming: the wins come from rethinking the algorithm to match the hardware constraints, not from micro-optimizing a fixed algorithm. The naive attention implementation does the right math. FlashAttention does the same math in a different order — an order that respects the memory hierarchy. That's the whole trick.

The 7.7× speedup at seq=4096 isn't free. It's the payoff for understanding that "memory bandwidth is the bottleneck" means something concrete, and that the path from O(N²) to O(N) memory is paved with tile boundary handling, online softmax updates, and careful shared memory layout. Writing the kernel teaches you why the speedup is possible in a way that reading the FlashAttention paper doesn't.
...
>
  <p>
    The original FlashAttention paper (NeurIPS 2022). Section 3 derives the online softmax algorithm and the tiling scheme from first principles. Reading this before implementing is essential — it explains *why* the algorithm works, not just the mechanics.
  </p>
</BookCard>
