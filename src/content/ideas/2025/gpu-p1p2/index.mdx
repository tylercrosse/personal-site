...
status: 'complete'
type: 'project'
tags: ['GPU', 'CUDA', 'shared-memory', 'tiling', 'matrix-multiplication', 'bitonic-sort', 'parallel-algorithms', 'NSight', 'MSCS']
category: ['projects']
draft: false
audience: 'Technical'
...
Together they teach the mental model that underlies almost all high-performance GPU code: understand where your bottleneck is (compute, global memory, or transfer), and choose the data layout and memory tier accordingly.

> [!note]
> This post focuses on the concepts and design challenges behind these two projects. For a broader overview of all the course material, see the companion post: [GPU Hardware and Software: A Retrospective](/ideas/2026/gpu-retro/).
...
<Figure
  ...
  caption="The CUDA memory hierarchy. Registers and shared memory live on-chip and are fast. Global memory (DRAM) is large but high-latency. The key pattern: load data from global into shared, compute in shared, write results back to global."
  width={640}
/>

- **Registers**: Fastest, private to each thread. Spilling to local memory (when you run out) is expensive.
- **Shared memory** (`__shared__`): Fast on-chip scratchpad, scoped to a thread block. The primary tool for data reuse. Roughly 100× faster than global memory for concurrent accesses when bank conflicts are avoided.
- **Global memory**: Large (tens of GBs) but high latency (~400–800 cycles). Every thread in the grid can access it. This is the dominant cost to manage.
...
The compute kernel pattern that almost all high-performance code follows:
...
A **bitonic sequence** is one that first strictly increases and then strictly decreases (or vice versa). The key property: a bitonic sequence can be sorted by repeatedly splitting it into two smaller bitonic sequences—a "bitonic split"—until you're left with sorted pairs, then merging up.
...
The XOR operation to find the partner index is the key. It computes the index of the element that each position should compare-swap with at each sub-stage. Because these indices are entirely determined by the loop variables—not by the data—the comparator network is fixed. No branch divergence from data-dependent decisions.
...
- **Pinned (page-locked) memory** (`cudaMallocHost` or `cudaHostRegister`): Allocating host memory as pinned removes the intermediate copy that the DMA engine needs to perform when transferring from pageable host memory. Transfers from pinned memory are typically 2× faster.
...
- **When is pinned memory actually faster?** For small transfers the overhead of pinning outweighs the benefit. For large transfers (hundreds of MB and up) it's almost always a win.

The NSight Compute feedback loop was central to this project. The pattern: run, profile, identify the dominant bottleneck (was it achieved occupancy, memory throughput, or divergent branches?), address it, and repeat. Each iteration taught something about what the hardware actually cares about.
