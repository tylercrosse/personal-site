...
OpenAI's Codex app takes a similar approach. When you create a new thread you choose between "Local" (direct project work) or "Worktree" (isolated changes). Automations, Codex's scheduled background tasks, [run in dedicated worktrees automatically](https://developers.openai.com/codex/app/features). Same pattern: give each agent its own working directory so parallel tasks can't collide.

This isn't limited to specific tools. Any agent framework that needs filesystem isolation can use worktrees the same way. The pattern is general: create a worktree per task, let the agent work, merge the result.
...
- The [official documentation](https://git-scm.com/docs/git-worktree) covers all flags and edge cases.
- [Atlassian's worktree guide](https://www.atlassian.com/git/tutorials/git-worktree) has some useful diagrams if you want a more visual explanation of how they work.
