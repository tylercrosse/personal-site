...
To ensure you get full credit, structure your submission with these three distinct sections. Make it easy for the grader to find each part. _Note: Correctness is never assumed from the algorithm steps; it must be explicitly argued in its own section._
...
1. **Identify the Pattern:** Determine if your problem maps to a standard algorithm.
    ...
    - _Polynomials, convolution, or multiplication_ usually point to Fast Fourier Transform (FFT).
...
T(n) =
\begin{cases}
O(n^{\log_b(a)}) &\text{if } n^{\log_b(a)} > n^d \quad\text{ ``leaf heavy"} \\
O(n^d \log(n)) &\text{if } n^{\log_b(a)} = n^d \quad\text{ ``balanced"}\\
O(n^d) &\text{if } n^{\log_b(a)} < n^d \quad\text{ ``root heavy"} \\
\end{cases}
$$
- **Case 1 (Leaf-Heavy):** $n\log_b​(a)>n^d$. The recursive branching explodes, meaning the vast majority of the work happens at the bottom of the tree. The leaves win, so the bound is strictly the number of leaves: $Θ(n\log_b​(a))$.

- **Case 2 (Perfectly Balanced):** $n\log_b​(a)=n^d$. It is a tie. The local work at the root matches the work at the leaves, and the work across every internal level of the tree is identical. Because the work per level is constant, you just multiply the root work by the number of levels ($\log n$). The bound is: $Θ(n^d\log n)$.
    
- **Case 3 (Root-Heavy):** $n\log_b​(a) < n^d$. The local work dwarfs the branching. The work shrinks geometrically as you go down the tree, meaning the total time is dominated by the massive chunk of work done at the very beginning. The root wins, so the bound is strictly the root work: $Θ(n^d)$.
