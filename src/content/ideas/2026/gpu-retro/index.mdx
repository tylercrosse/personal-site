...
- **Extra Project:** [FlashAttention & LLM Inference on GPUs](/ideas/2026/gpu-p6/)
...
A recurring pattern in high-performance CUDA kernels is: load from global into shared, compute, then write back to global. The tiled matrix multiplication in [Project 1](/ideas/2026/gpu-p1p2/) is the canonical example.
...
Tiled matrix multiplication ([Project 1](/ideas/2026/gpu-p1p2/)) is a classic example of using shared memory to increase arithmetic intensity and push a workload from memory-bound to compute-bound. The FlashAttention algorithm ([Extra Project](/ideas/2026/gpu-p6/)) uses the same principle at much larger scale.
...
- **Extra Project (FlashAttention)**: A progressive implementation of self-attention, FlashAttention-2, and a KV-cached decode kernel inside a real GPT-2 inference pipeline. The capstone. [Full write-up.](/ideas/2026/gpu-p6/)
